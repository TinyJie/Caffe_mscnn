...
void DetectionLayer<Dtype>::LayerSetUp(
  ...
  DetectionParameter detect_param = this->layer_param_.detection_param();
...
void DetectionLayer<Dtype>::Forward_cpu(
  ...
  const Dtype min_whr = log(Dtype(1)/field_whr_), max_whr = log(Dtype(field_whr_));
  const Dtype min_xyr = Dtype(-1)/field_xyr_, max_xyr = Dtype(1)/field_xyr_;
  ...
  if (normalize_) {
    ...
    cls_loss /= num;
    coord_loss = coord_loss / num / coord_num_;
  }
...
template <typename Dtype>
void DetectionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < num; ++i) {
      if (normalize_) {
        ...
        caffe_scal(cls_dim, loss_weight / num, bottom_diff+i*bottom_dim);
        caffe_scal(coord_dim, loss_weight*lambda_ / num / coord_num_, bottom_diff+i*bottom_dim+cls_dim);
      }
    }
