...
template <typename Dtype>
void DetectionLayer<Dtype>::Forward_gpu(
...
template <typename Dtype>
void DetectionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < num; ++i) {
      if (normalize_) {
        ...
        caffe_gpu_scal(cls_dim, loss_weight / num, bottom_diff+i*bottom_dim);
        caffe_gpu_scal(coord_dim, loss_weight*lambda_ / num / coord_num_, bottom_diff+i*bottom_dim+cls_dim);
      }
    }
  }
}

INSTANTIATE_LAYER_GPU_FUNCS(DetectionLayer);
